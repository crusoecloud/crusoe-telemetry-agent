...
REMOTE_CRUSOE_TELEMETRY_SERVICE="vm/systemctl/crusoe-telemetry-agent.service"
...
do_install() {
  ...
  if $HAS_NVIDIA_GPUS; then
    ...
    wget -q -O "$CRUSOE_TELEMETRY_AGENT_DIR/dcp-metrics-included.csv" "$GITHUB_RAW_BASE_URL/$REMOTE_DCGM_EXPORTER_METRICS_CONFIG" || error_exit "Failed to download $GITHUB_RAW_BASE_URL/$REMOTE_DCGM_EXPORTER_METRICS_CONFIG"
...
do_uninstall() {
  ...
  status "Removing telemetry agent files."
